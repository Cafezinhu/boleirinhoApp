...
class IngredienteNaReceitaDao{
  ...
  Future<List<IngredienteNaReceita>> find(int receitaId) async{
    ...
    List<Map<String, dynamic>> result = await db.rawQuery('SELECT * FROM $_tableName WHERE $_receitaId=?', [receitaId]);
  ...
  Future<List<IngredienteNaReceita>> _toList(List<Map<String, dynamic>> mapList) async{
    List<IngredienteNaReceita> list = List();
    for(Map<String, dynamic> map in mapList){
      List<Ingrediente> ingredientes = await IngredienteDao().findById([map[_ingredienteId]]);
      Ingrediente ingrediente = ingredientes[0];
      list.add(IngredienteNaReceita(ingrediente, double.parse(map[_quantidade])));
    }

    return list;
  }

  Future<List<int>> save(List<IngredienteNaReceita> ingredientesNaReceita, int receitaId) async{
    Database db = await getDatabase();
    List<Map<String, dynamic>> mapList = _toMapList(ingredientesNaReceita, receitaId);
    List<int> ids = List();
    for(Map<String, dynamic> map in mapList){
  ...
  List<Map<String, dynamic>> _toMapList(List<IngredienteNaReceita> list, int receitaId){
